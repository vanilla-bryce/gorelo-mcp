...
for i, c in enumerate(CLIENTS):
    for j in range(4):
        AGENTS.append({
            ...
            "Name": f"WS-{i:02d}{j}",
            "OperatingSystem": "Windows 10 Pro" if j == 0 else "Windows 11 Pro",
...
def make(n, status, client, title, lead=ME, assisting=None, watchers=None,
    ...
    t = {
        ...
        "TagIds": [], "IsUnread": False, "IsAwaitingClient": False,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/assets/agents":       # no client filter, as in real Gorelo
            rows, pag = paginate(AGENTS, q)
            return self.reply(200, env(rows, pag))
    ...
    def do_POST(self):
        ...
        m = re.fullmatch(r"/v1/tickets/([^/]+)/(comments|conversations|notes)", u.path)
        if m and m.group(2) == "comments":
            POSTED.append({"ticket": m.group(1), "body": body})
            sys.stderr.write("POSTED to %s internal=%s\n" % (m.group(1), body.get("IsInternal")))
            sys.stderr.flush()
